...
class GamaLocalAdjParserError(Error):
    pass
...
class GamaLocalAdjParser(object):
    ...
    def __init__(self, data, type="all"):
        '''
        data : Network instance for data storage
    ...
    def _handle_end_element_all(self, name):
        ...
        if name == "description":
            self._description = self._characterData.strip()
            ...
            if self._tagLast == "adjusted":
                ...
                if self._coordX is not None:
                    index[0] = self._covmatrow
                    self._covmatrow += 1
                if self._coordY is not None:
                    index[1] = self._covmatrow
                    self._covmatrow += 1
                if self._coordZ is not None:
        ...
        # project equations
        elif name == "equations" or name == "unknowns" or name == "defect":
...
if __name__ == "__main__":
    ...
    print "see gizela.data.NetworkAdj.py for example"
