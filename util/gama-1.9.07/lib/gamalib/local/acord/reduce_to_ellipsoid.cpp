...
ReduceToEllipsoid::EllipsoidFunction::EllipsoidFunction(GNU_gama::Ellipsoid EL, double lat, LocalPoint& cpoint):
    el(EL), latitude(lat), centralPoint(cpoint)
{
    r = R();
}
...
double ReduceToEllipsoid::EllipsoidFunction::central_angle12(const LocalPoint& p2) const
{
    ...
    //std::cerr << "z for central angle: " << z2 << endl;
...
ReduceToEllipsoid::ReduceToEllipsoid(PointData& b, ObservationData& m,
				     ...
				     LocalPoint& cpoint):
    PD(b), OD(m), EL(el), latitude(lat),  EF(EllipsoidFunction(el, lat, cpoint))
{
...
bool ReduceToEllipsoid::reduce_z_angle_to_ellipsoid(Z_Angle* obs)
{
    ...
    double ca = EF.central_angle12(p2);
    
    //double bearing = bearing23 - bearing21;
    double bearing = bearing23 - bearing21 + M_PI;

    double correction = +ca*cos(bearing);
                  //  = -EF.central_angle23(p2,p3)*cos(bearing23 - bearing21);
		  
    reduced[obs] = obs->value();
    obs->set_value( obs->value() + correction);

    /*std::cerr << "z-angle from = " << obs->from() << " to = " << obs->to() << endl
              << "  central angle(gon) = " << ca * 200 / M_PI << endl
...
bool ReduceToEllipsoid::reduce_direction_to_ellipsoid(Direction* obs)
{
    ...
    for (ObservationData::iterator i=OD.begin(), e=OD.end(); i!=e; ++i)
    {
        ...
        /*if ( !o->active() ) {
	    std::cerr << "unactive obs: from=" << o->from() << " to=" << o->to() << endl;
            continue;
	}*/
        ...
        if (/* Z_Angle* z =*/ dynamic_cast<Z_Angle*>(o) )
        {
            ...
            if ( (p2id == from) && (p3id == to) )
            {
                zenithSum += o->value();
                ++zenithNum;
		//std::cerr << "zangle for reduction: from=" << from << " to=" << to << endl;
            }
            else
		...
		/*else {
			std::cerr << "unused zangle: from=" << from << " to=" << to << endl;
		}*/
        }
    }
    ...
    double correction = -ca/tan(zenith23)*sin(bearing);

    /*std::cerr << "direction from = " << p2id << " to = " << p3id << endl
              << "  central angle(gon) = " << ca * 200 / M_PI << endl
              << "  zenith angle (gon) = " << zenith23 * 200 / M_PI << endl
	      << "  bearing      (gon) = " << bearing * 200 / M_PI << endl
	      << "  correction   (gon) = " << correction * 200 / M_PI << endl;
    */
...
LocalPoint ReduceToEllipsoid::execute()
{
    if ( !reduced.empty() )
        reduced.clear();
    ...
    //return LocalPoint::XYZ(centralX, centralY, centralZ);
    return EF.getCentralPoint();
